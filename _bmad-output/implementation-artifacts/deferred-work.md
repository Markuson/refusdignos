...
- source_spec: `_bmad-output/implementation-artifacts/spec-2-4-manage-unete-page-copy-via-keystatic.md`
  summary: None of `unete.astro`'s `.map()` calls over `unetePage`'s five array fields guard against the field being missing/undefined (e.g. `(unetePage.faqItems ?? []).map(...)`), so a hand-edited or corrupted `index.json` would throw a build-time TypeError instead of failing gracefully.
  evidence: Same unguarded pattern already exists in `proyecto.astro`'s three array `.map()` calls (`valores`, `impactoChecklist`, `impactoStats`) from Story 2.3, so this isn't a regression introduced by this story -- it's the established repo convention for JSON-singleton array fields, which rely on Keystatic's `isRequired`/`length.min` validation to guarantee the field exists at save time. Confirmed via the story 2.4 review's edge-case-hunter pass.

- source_spec: `_bmad-output/implementation-artifacts/spec-2-4-manage-unete-page-copy-via-keystatic.md`
  summary: The `sprint-status.yaml` story key `2-4-manage-únete-page-copy-via-keystatic` is the only development_status key containing an accented character; every sibling key is a plain-ASCII slug (some truncated to fit, e.g. `2-2-manage-global-settings-homepage-and-contact-page-copy-via-ke`).
  evidence: Pre-existing since the key was already present (as `backlog`) before this story touched the file -- this story only flipped its value to `in-progress`, it didn't introduce the key. Worth normalizing to ASCII (`2-4-manage-unete-page-copy-via-keystatic`) next time `sprint-status.yaml` gets a broader edit, in case any tooling assumes ASCII-only keys. Confirmed via the story 2.4 review's blind-hunter pass.
