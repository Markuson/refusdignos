...
- **Access control:** a GitHub App with an `allowedUsers` list restricts who can authenticate into `/keystatic`; accounts not on the list are denied access.
...
- **Required env vars:** `PUBLIC_KEYSTATIC_GITHUB_REPO`, `PUBLIC_CLOUDINARY_CLOUD_NAME`, `PUBLIC_CLOUDINARY_UPLOAD_PRESET` — all `PUBLIC_`-prefixed by design since both the admin UI and the browser-to-Cloudinary upload run client-side; document them in `.env.example`.
